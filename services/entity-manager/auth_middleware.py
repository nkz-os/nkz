    ...
    def decorated(*args, **kwargs):
                ...
                if KEYCLOAK_AUTH_AVAILABLE:
                    try:
                        ...
                        if not g.email or g.email == "unknown":
                            logger.warning(
                                "Email not found in token payload. Available fields: %s",
                                list(payload.keys()),
                            )
        ...
        if api_key:
            ...
            if api_key_info and api_key_info.get("valid"):
                _raw_tenant = api_key_info["tenant_id"]
                try:
                    from tenant_utils import normalize_tenant_id

                    g.tenant = normalize_tenant_id(_raw_tenant)
                except (ImportError, ValueError):
                    g.tenant = _raw_tenant
...
def log_entity_operation(operation, entity_type, entity_id=None):
    ...
    logger.info(
        f"[AUDIT] {operation} {entity_type} {entity_id or 'N/A'} by {user} in tenant {tenant}"
    )
