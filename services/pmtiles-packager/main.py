...
MINIO_ACCESS_KEY = os.getenv(
    "MINIO_ROOT_USER", os.getenv("MINIO_ACCESS_KEY", "minioadmin")
)
...
try:
    candidate_paths = [
        "/app/services/task-queue/task_queue.py",  # monorepo root build context
        "/app/task-queue/task_queue.py",  # legacy path
    ]
    task_queue_file = next((p for p in candidate_paths if os.path.exists(p)), None)
    ...
    if task_queue_file:
        ...
        logger.info("TaskQueue module loaded successfully from %s", task_queue_file)
    else:
        logger.warning("task_queue.py not found in expected paths: %s", candidate_paths)
        ...
        try:
            ...
            try:
                ...
                self.s3.put_bucket_policy(
                    Bucket=MINIO_BUCKET, Policy=json.dumps(policy)
                )
            except Exception as e:
...
class PMTilesPackager:
    def __init__(self):
        ...
        if TaskQueue:
            ...
            self.status_queue = TaskQueue(
                stream_name="pmtiles:status"
    ...
    def build_package(self, parcel_id: str, bbox: list, max_zoom: int) -> str:
        ...
        """
        logger.info(
            f"Building PMTiles for parcel {parcel_id} with bbox {bbox} up to zoom {max_zoom}"
        )
    ...
    def process_task(self, task: dict):
        ...
        try:
            ...
            if self.queue:
                self.queue.update_task_status(
                    task.get("id"), status="failed", error_message=str(e)
                )
