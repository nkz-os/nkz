...
def should_overwrite_location(
    current_location: Optional[Dict], new_location: Dict
) -> bool:
    """
...
def map_isobus_to_ngsi_ld(
    ...
    ngsi_payload: Dict[str, Any] = {"@context": [CONTEXT_URL]}
    ...
    try:
        ...
        if not overwrite_location and "location" in ngsi_payload:
            ...
            if existing_entity and "location" in existing_entity:
                ...
                if not should_overwrite_location(
                    existing_location, ngsi_payload["location"]["value"]
                ):
                    # No sobreescribir, mantener la existente
                    del ngsi_payload["location"]
                    logger.info(
                        f"Keeping existing location for {entity_id}, GPS data ignored"
                    )
        ...
        if response.status_code in (204, 200):
            logger.info(
                f"Successfully updated entity {entity_id} for tenant {tenant_id}"
            )
            return True
        else:
            logger.error(
...
def create_entity_if_not_exists(
    ...
    try:
        entity_payload = {
            ...
            "type": entity_type,
            "@context": [CONTEXT_URL],
            "name": {"type": "Property", "value": device_id},
            "location": {
                "type": "GeoProperty",
                "value": {
                    "type": "Point",
                    "coordinates": [
                        initial_location.get("lon", 0),
                        initial_location.get("lat", 0),
                    ],
                },
                "metadata": {
                    "location_source": {
...
def health():
    ...
    return jsonify(
        {"status": "healthy", "service": "isobus-bridge", "orion_url": ORION_URL}
    ), 200
...
def receive_isobus_telemetry():
    ...
    Headers requeridos:
        X-API-Key: API Key del tenant (la API key es la única fuente de
                   verdad para el tenant_id — no se acepta Fiware-Service
    ...
    try:
        ...
        # The API key is the canonical source of truth for tenant_id.
        # The legacy 'Fiware-Service' header override was removed
        # (2026-05-29): it accepted an arbitrary header value AFTER auth
        # and overwrote the validated tenant_id, allowing any client with
        # a valid API key to write telemetry against any other tenant
        # (horizontal privilege escalation). Each ISOBUS entity belongs
        # to exactly one tenant, so there is no legitimate multitenant
        # use case for the override.

        # 2. Validar payload JSON
        if not request.is_json:
            return jsonify(
                {"error": "Invalid content type", "detail": "Request must be JSON"}
            ), 400
        ...
        if not device_id:
            return jsonify(
                {"error": "Missing required field", "detail": "deviceId is required"}
            ), 400

        location_data = data.get("location", {})
        if (
            not location_data
            or "lat" not in location_data
            or "lon" not in location_data
        ):
            return jsonify(
                {
        ...
        if existing_entity and "location" in existing_entity:
            overwrite_location = should_overwrite_location(
                existing_entity["location"], location_data
            )

        # 7. Crear entidad si no existe
        if not existing_entity:
            create_entity_if_not_exists(
                entity_id, entity_type, device_id, tenant_id, location_data
            )

        # 8. Mapear datos ISOBUS a NGSI-LD
        ngsi_payload = map_isobus_to_ngsi_ld(device_id, data, tenant_id, entity_type)

        # 9. Actualizar entidad en Orion-LD
        success = update_entity_in_orion(
            entity_id, tenant_id, ngsi_payload, overwrite_location
        )
