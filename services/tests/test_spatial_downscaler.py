...
class TestSolarRadiationAspect:
    ...
    def test_flat_terrain_no_correction(self):
        ...
        result = correct_solar_radiation_aspect(800.0, 42.0, 180.0, 0.5, 180)
        assert result == 800.0
    ...
    def test_north_facing_less_in_winter(self):
        """North-facing slope gets less radiation than south-facing in winter."""
        # Use winter solstice (day 355) at 50°N — dramatic difference
        result_north = correct_solar_radiation_aspect(500.0, 50.0, 0.0, 20.0, 355)
        result_south = correct_solar_radiation_aspect(500.0, 50.0, 180.0, 20.0, 355)
        if result_north is not None and result_south is not None and result_south > 0:
            # South should get more than north in winter (northern hemisphere)
            assert result_south > result_north, (
                f"South={result_south} should be > North={result_north}"
            )
...
class TestRecalculateDeltaT:
    ...
    def test_warm_dry_high_delta_t(self):
        ...
        # At 25°C / 40% RH, delta-t is ~5.8°C (Magnus psychrometric)
        assert 4.0 < dt < 10.0, f"Got {dt}"
