...
export const ViewerHeader: React.FC = () => {
    ...
    const [isLayersExpanded, setIsLayersExpanded] = useState<boolean>(() => {
        try {
            const v = localStorage.getItem('nkz.viewer.layersMenu.expanded');
            return v === null ? true : v === 'true';
        } catch { return true; }
    });
    useEffect(() => {
        try { localStorage.setItem('nkz.viewer.layersMenu.expanded', String(isLayersExpanded)); } catch { /* quota or private mode */ }
    }, [isLayersExpanded]);
            ...
            >
                ...
                >
                    ...
                    <div className="py-2 border-t border-slate-200 dark:border-slate-700">
                        <button
                            type="button"
                            onClick={() => setIsLayersExpanded(v => !v)}
                            className="w-full px-4 py-1 flex items-center justify-between text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
                            aria-expanded={isLayersExpanded}
                            aria-label={isLayersExpanded ? t('viewer.layersCollapse') : t('viewer.layersExpand')}
                        >
                        ...
                        {isLayersExpanded && (
                            <div className="px-2 pb-2 space-y-1">
                                ...
                                >
                                    ...
                                    <span className={`text-xs px-1.5 py-0.5 rounded ${riskEnabled ? 'bg-red-100 text-red-600' : 'bg-slate-200 text-slate-500'}`}>
                                        {riskEnabled ? 'ON' : 'OFF'}
                                    </span>
                                </button>
