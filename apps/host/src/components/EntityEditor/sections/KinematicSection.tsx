...
export const KinematicSection: React.FC = () => {
  const { t } = useI18n();
  const { formState, setField, entityType } = useEntityEditor();
  const schemas = getSchemasForType(entityType).filter(s => s.section === 'kinematic');
  ...
  return (
    <details className="border border-gray-200 rounded-lg">
      <summary className="px-4 py-3 bg-gray-50 cursor-pointer text-sm font-semibold text-gray-700 hover:bg-gray-100 rounded-lg">
        {t('editor.section.kinematic') || 'Cinemática y enganche'}
      </summary>
